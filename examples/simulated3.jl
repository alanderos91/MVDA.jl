...
function compute_FPR(TP, FP, TN, FN)
    v = FP / (TP + FP)
...
function run_experiment(n, p, c, nsamples)
    ϵ = 0.5 * sqrt(2*c/(c-1))
    s_grid = [range(0.0, 0.98, length=15); 1.0]
    B0 = zeros(p, c-1)
    B0[1:2, :] .= 1
    d_vals = (1.0, 2.0, 3.0)
    ...
    for (j, d) in enumerate(d_vals)
        for ii in 1:N
            ...
            for (i, s) in enumerate(s_grid)
                fit_MVDA!(MMSVD(), problem, ϵ, s, extras, (true,true), verbose=false, nouter=1000, ninner=10^6, rtol=0.0, gtol=1e-8, dtol=1e-6, nesterov_threshold=100);
                Y = problem.Y
                B = problem.proj.all
                V = unique(Y, dims=1)
                ...
                tmp2[i,ii] = 100 * compute_FPR(metrics...)
...
savefig(fig, "~/Desktop/VDA/simulated3A.png")
...
p = 100
fig = run_experiment(n, p, c, nsamples)
savefig(fig, "~/Desktop/VDA/simulated3B.png")
